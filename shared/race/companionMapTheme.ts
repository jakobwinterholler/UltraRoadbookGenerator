/** Premium purple route branding for Companion maps. */
export const ROUTE_HALO_COLOR = "#a78bfa";
export const ROUTE_CORE_COLOR = "#7c3aed";
export const ROUTE_HALO_WIDTH = 12;
export const ROUTE_CORE_WIDTH = 6;
export const ROUTE_HALO_OPACITY = 0.5;
export const ROUTE_CORE_OPACITY = 1;

export const SELECTED_STOP_HALO = "#a78bfa";
export const SELECTED_STOP_CORE = "#7c3aed";
export const ALTERNATIVE_STOP_COLOR = "#64748b";
export const DIMMED_STOP_COLOR = "#475569";
export const SUGGESTED_STOP_COLOR = "#E85D04";
export const SKIPPED_STOP_COLOR = "#9ca3af";

/** Street-level focus — matches the pre-overhaul detail map feel. */
export const POI_FOCUS_ZOOM = 17.8;
export const POI_FOCUS_ANIMATION_MS = 480;
export const POI_FOCUS_OFFSET: [number, number] = [0, -90];
export const DETAIL_MAP_ZOOM = 17.8;
export const DETAIL_ROUTE_WINDOW_KM = 0.12;
